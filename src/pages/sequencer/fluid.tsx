...
export function addNoteCallback(note: Note, fluidInitialObjects: InitialObjects, undoRedoHandler: UndoRedoStack) {
...
export function removeNoteCallback(note: Note, fluidInitialObjects: InitialObjects, undoRedoHandler: UndoRedoStack) {
    const flNotes = fluidInitialObjects?.sequence as SharedMap;
    const key = note.getNoteKey().serialize();
    const prevValue = flNotes.get(key);
...
export function removeAndAddNoteCallback(rmNote: Note, addNote: Note, fluidInitialObjects: InitialObjects, undoRedoHandler: UndoRedoStack) {
    const flNotes = fluidInitialObjects?.sequence as SharedMap;
...
export function removeAddMultipleCallback(rmNotes: Note[], addNotes: Note[], fluidInitialObjects: InitialObjects, undoRedoHandler: UndoRedoStack) {
    const flNotes = fluidInitialObjects?.sequence as SharedMap;
